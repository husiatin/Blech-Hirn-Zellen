...
export function assembleBoard(choices) {
  ...
  for (const config of quadrantConfigs) {
    ...
    const sideChips = CHIP_POSITIONS[config.blockName][chosenSide];

    for (const chip of sideChips) {
      gameInfo.chips.push(new Chip(chip.x + config.offsetX, chip.y + config.offsetY, chip.color, chip.symbol));
    }
